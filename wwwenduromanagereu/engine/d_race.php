		...
		if ($list){		
			...
			$where = "  $where (
						exists(
							select * 
							from `b_racecrew` rc 
							where (rc.raceid = r.race_id ) and rc.`userid` = ".$_SESSION['user']['user_id']."
						) or
						exists (
							select * 
							from `phpbb_user_group` ug
								inner join `phpbb_groups` g on (g.`group_id` = ug.`group_id`)
							WHERE ug.`user_id` = ".$_SESSION['user']['user_id']." and (g.`group_name` = '$group' or g.`group_name` = '".RM_ADMINS."')
						)
					)";
		}
...
function printRace(){	
	$sec = new Security();
	if(!$sec->testUserGroup(1,'RM_ENDURO_ADMIN')){
