function changeNumber(ea, cl, nr) {
	var newnr = prompt('Ievadiet sportista numuru!', nr);
	if (newnr != null) {
		window.location = "?rm_func=enduro&rm_subf=changeNR&ea="+ea+"&nr="+newnr;
	}
}
...
function showApplMenu(obj, hs) {
	...
	//alert(xmlhttp.responseText);
	x = document.getElementById("div_apl");

	x.innerHTML = "text text";//xmlhttp.responseText;

	alert(x.innerHTML);
	hs.htmlExpand(obj, { headingText: 'Pieteikums sacensībam' });
}