...
class EnduroManager{
	...
	public function getFreeRacerList($r){
		...
		$sql="	SELECT `user_id`,
				...
				WHERE 
					pd.`user_id` not in (select racer_id from `enduro_application` where `race_id` = $r) and 
					pd.`user_id` in (select `user_id` from `phpbb_user_group` where `group_id` = ".RACER_GROUP_ID.")
				ORDER BY NR
		";
		//cast(pd.`pf_rm_sport_nr` as decimal(5,2)) as NR
		//echo $sql;
...
function proceedEnduro($subf,$opt){
	...
	switch($subf){
		...
		case "clubapply":
			...
			break;
		case "addapplication":	
			$_SESSION['params']['racer'] = $_SESSION['params']['racer']?$_SESSION['params']['racer']:$_SESSION['user_id'];
...
function printEnduroApply($opt){
	...
	//$apl = $em->getERA("",$opt,"",$_SESSION['user_id'],"");	
	...
	$user = $_SESSION['params']['rcr'] ? $_SESSION['params']['rcr'] : $_SESSION['user_id'];
	...
	if(!$apl[0]){
		...
		//echo "<tr><td>Apdrošināšana:<td>",$apl[0]->INS ? "Ir":"Nav";				
	
		//echo "<tr><td colspan=\"2\"><a href=\"index.php?rm_mode=print&print_func=enduroapl&apl=",$apl[0]->ERA_ID," \" target=\"_blank\">Drukāt anketu</a>";
...
function printEnduroClubApply($opt){
	...
	$manager = $rcm->getRacer($_SESSION['user_id']);
				...
				echo "<table border = \"1\" width=\"100%\" valign=\"top\" style=\"background-color:white\">";
					for($i=0;$i<count($racers);$i++){
						...
						echo ">";
							...
							echo "<td width=\"25\">";
								echo "<a href=\"http://www.appasaule.lv/rm/?rm_mode=print&print_func=enduroapl&apl=",$racers[$i]->EA_ID,"\" target=\"_blank\">";
									echo "<img src=\"./images/ANKETA_24x24.png\" border=\"0\" alt=\"Drukāt\" title=\"Drukāt anketu\">";
								echo "</a>";
							
							echo "<td width=\"*\">";
								echo "<font style=\"font-weight:bold;font-size:14px\">",$racers[$i]->RACER->getNR() ? $racers[$i]->RACER->getNR() : "<font color=\"red\">NAV</font>" ,"</font> / ";
...
function printStarts($opt){
	...
	echo "<hr><a href=\"http://appasaule.lv/rm/?rm_mode=print&print_func=rudite&day=$day&mode=1\" target=\"_blank\">Protokols \"Rudītes\"</a>";
	echo " | <a href=\"http://appasaule.lv/rm/?rm_mode=print&print_func=rudite&day=$day&mode=2\" target=\"_blank\">Protokols \"Robis\"</a>";
	echo " | <a  href=\"http://appasaule.lv/rm/?rm_mode=print&print_func=enduro_start&day=$day\" target=\"_blank\">Starta laiki</a>";
	echo " | <a  href=\"http://appasaule.lv/rm/?rm_mode=print&print_func=kartina&day=$day\" target=\"_blank\">Kartiņas</a>";
...
function pairRacer(){
	...
	$cnt = count($racers) > 0 ? count($racers): 1;
