...
class RacerManager{
	...
	//echo $sql;
		...
		if (count($reslt)<1 && $id <> ""){
			...
			array_push($reslt,$item);
		//	$sql="insert into `phpbb_profile_fields_data` (user_id) values ($id);";
		//	echo $sql;
		//	//queryDB($sql);	
			//return $this->getRacer($id);
		}
...
function saveNewUser(){
	...
	//require_once( "../phpBB3/includes/utf/utf_tools.php");	
	...
	if ($_SESSION['params']['addmode']=="enduroreg"){
		...
		if (!$_SESSION['params']['hasmail']){
			...
			$i = 0;
			while(!$rcm->chechName($name.($i ? $i : "")."@apPasaule.lv")){						
				$i++;
			}				
			$_SESSION['params']['email'] = $name.($i ? $i : "")."@apPasaule.lv";
		}
	}		
	
	if (!$rcm->chechName(trim($_SESSION['params']['email']))){
	...
	$user_row = array(
		...
		'user_password'			=> phpbb_hash(trim($_SESSION['params']['p1'])),
	...
	sendMail($_SESSION['params']['email'],"","Reģistrācija portālā www.apPasaule.lv",$text);
...
function imp() {
	...
	$arr = file("engine/us.txt");
	foreach ($arr as $line_num => $line) {
	
		$data = explode(";",$line);
	
		if (!$rcm->chechName($data[0])){
			echo "Lietotājs <b> ",$data[0]," </b> jau eksistē!<br>";
			continue; 
		} 
			
		$user_row = array(
			'username'				=> $data[0],
			'user_password'			=> phpbb_hash($data[1]),
			'user_email'			=> $data[0],
			'group_id'				=> USER_GROUP_ID,
			'user_timezone'			=> 0.00,
			'user_dst'				=>  0,
		...
		$cp_data = array(
			...
			'pf_rm_l_name'			=> $data[3],
			'pf_rm_pk'				=> $data[9],		
			'pf_rm_address'			=> $data[4],
			'pf_rm_phone'			=> $data[5],
			'pf_rm_sex'				=> $sex,			
			'pf_rm_moto_type'		=> $tehn,
			'pf_rm_moto_name'		=> $data[11],
			'pf_rm_sport_nr'		=> $data[16],			
			'pf_rm_lic_nr'			=> $data[18],
			'pf_rm_moto_v'			=> $data[13]*=1,			
		);
		...
		echo "Lietotājs <b> ",$data[0],":",$data[1]," </b> izveidots!<br>";
	}
	//print_r($arr);
...
function printInfo(){
	...
	$racer = $rcm->getRacer($_SESSION['user_id']);
			...
			echo "<tr>";					
				...
				if ($resnr!= 1111){ 
					...
					echo"<td >";
						...
						echo "</select>";	
					//echo "<select name = \"CLUB".$rcrs[$i]->getRacerID()."\" style=\"width: 200px;\" ></select>";
...
function printProfile($opt){
	...
	echo "<table width =\"100%\" border = \"1\">";
		...
		//<input type=\"text\" name = \"nr\" value=\"",$racer[0]->getNR(),"\">
		...
		if($sec->testUserGroup($_SESSION["user_id"],ENDURO_ADMINS) || $sec->testUserGroup($_SESSION["user_id"],RM_ADMINS)){
		...
		echo "<tr><td>Apdrosinasana: <td ><input type=\"checkbox\" name =\"ins\"";
			if ($racer[0]->getIns() == 1){echo " checked ";}
		echo ">";
		
		echo "<tr><td>Tehnikas tips: <td ><select name =\"tt\">";
			echo "<option value=\"1\"";
				if ($racer[0]->getTehn() == 1){echo " selected ";};
			echo ">&lt;Nav izvēlēts&gt;</option>";
			echo "<option value=\"2\"";
				if ($racer[0]->getTehn() == 2){echo " selected ";};
			echo ">Moto</option>";
			echo "<option value=\"3\"";
				if ($racer[0]->getTehn() == 3){echo " selected ";};
			echo ">Velo</option>";
			echo "<option value=\"4\"";
				if ($racer[0]->getTehn() == 4){echo " selected ";};
			echo ">Kvadro</option>";
		echo "</select>";
		*/
		...
		echo "</select>";
		/*
		echo "<tr><td>Motora tilpums: <td ><input type=\"text\" name = \"motocc\" value=\"",$racer[0]->getMotocc(),"\">";
	*/	
...
function listMyTeam(){
	...
	if ($owner and count($rcrs) < 2 ){
		...
		//echo " | <a href=\"index.php?rm_func=racer&rm_subf=addteamracer&opt=",$team[0]->getID(),"\">Aicināt esošo</a>";
		echo "</center>";		
	}
}
...
function saveTeam($opt){
	...
	if ($opt > 0){
		...
		$tms = $rcm->getTeam("","","",$_SESSION['params']['name']);		
		$rcm->insTeamRacer($_SESSION['user_id'],$tms[0]->getID(),1)	;	
	}
}
...
function printNewTRacer($opt,$withdata){
	...
	echo "<table width =\"100%\" border = \"1\">";
		...
		if (
			...
			$sec->testUserGroup($_SESSION['user_id'],"RM_MPS_admins")
				...
				// echo "<tr><td>Starta numurs: <td ><input type=\"text\" name = \"nr\" value=\"";
				// if ($withdata){echo $_SESSION['params']['nr']};
				// echo "\">";
		...
		/*
		echo "<tr><td>Licences numurs: <td ><input type=\"text\" name = \"lnr\" value=\"";
		if ($withdata){echo $_SESSION['params']['lnr'];}
		echo "\">";
		
		$list = $rcm->getClub();
		echo "<tr><td>Klubs <td ><select type=\"text\" name = \"club\" >";
		for($i=0;$i<count($list);$i++){
			echo "<option value=\"",$list[$i]->getID()+1,"\">";
			echo $list[$i]->getName();
			echo "</option>";
		}
		echo "</select>";
	...
	/*	
		...
		echo "<tr><td>Tehnikas tips: <td ><select name =\"tt\">";
			echo "<option value=\"1\">&lt;Nav izvēlēts&gt;</option>";
			echo "<option value=\"2\">Moto</option>";
			echo "<option value=\"3\">Velo</option>";
			echo "<option value=\"4\">Kvadro</option>";
		echo "</select>";
		
		echo "<tr><td width=\"100\" >Marka: <td>";
		...
		for($i=0;$i<count($moto);$i++){
			echo "<option value=\"",$i+1,"\"";
				if ($withdata){if ($i+1 == $_SESSION['params']['tn']) {echo " selected ";};}
			echo ">",$moto[$i],"</option>";
		}
		...
		echo "<tr><td>Motora tilpums: <td ><input type=\"text\" name = \"motocc\" value=\"";
		if ($withdata){echo $_SESSION['params']['motocc'];}
		echo "\">";
		*/
	echo "</table> ";
	echo "<hr>";
